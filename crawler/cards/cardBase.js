...
import { getTargetDataPath } from '../env.js'
...
async function writeJSON (filename, source, task) {
  await fs.writeFile(`${getTargetDataPath('cards')}/${filename}.json`, JSON.stringify(source), 'utf8')
          .then(() => {
            task.output = `${getTargetDataPath('cards')}/${filename}.json Saved.`
          })
          .catch(err => {
